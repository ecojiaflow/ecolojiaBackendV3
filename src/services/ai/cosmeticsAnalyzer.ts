...
class CosmeticsAnalyzer {
  ...
  async analyzeEnvironmentalImpact(productData) {
    ...
    if (text.includes('biodegradable')) envAnalysis.score += 10;
    if (text.includes('recyclable')) envAnalysis.score += 8;
    if (text.includes('refill')) envAnalysis.score += 12;

    // Malus ingrédients problématiques environnement
    if (text.includes('silicone')) envAnalysis.score -= 8;
    if (text.includes('microplastic')) envAnalysis.score -= 15;
  ...
  assessComplexity(ingredientCount) {
    if (ingredientCount < 10) return 'Simple';
    if (ingredientCount < 20) return 'Modérée';
    if (ingredientCount < 30) return 'Complexe';
  ...
  detectFormulationType(productData) {
    ...
    if (text.includes('crème') || text.includes('cream')) return 'Émulsion';
    if (text.includes('gel')) return 'Gel';
    if (text.includes('huile') || text.includes('oil')) return 'Phase huileuse';
    if (text.includes('sérum')) return 'Solution concentrée';
  ...
  generateUsageRecommendations(productData) {
    ...
    if (text.includes('visage')) recommendations.push('Appliquer sur peau propre et sèche');
    if (text.includes('corps')) recommendations.push('Utiliser après la douche');
    if (text.includes('cheveux')) recommendations.push('Masser délicatement le cuir chevelu');
