...
class ProductTypeDetector {
  ...
  calculateFoodScore(data) {
    ...
    if (productMatches > 0) evidence.push(`${productMatches} produits alimentaires détectés`);

    // Catégories alimentaires
    const categoryMatches = this.countKeywordMatches(data.fullText, this.keywords.food.categories);
    score += categoryMatches * 15;
    if (categoryMatches > 0) evidence.push(`${categoryMatches} catégories alimentaires`);
    ...
    if (ingredientMatches > 0) evidence.push(`${ingredientMatches} ingrédients alimentaires`);
  ...
  calculateCosmeticScore(data) {
    ...
    score += productMatches * 20;
    if (productMatches > 0) evidence.push(`${productMatches} produits cosmétiques détectés`);

    // Catégories beauté
    const categoryMatches = this.countKeywordMatches(data.fullText, this.keywords.cosmetic.categories);
    score += categoryMatches * 15;
    if (categoryMatches > 0) evidence.push(`${categoryMatches} catégories beauté`);

    // Ingrédients INCI
    const inciMatches = this.countKeywordMatches(data.fullText, this.keywords.cosmetic.ingredients);
    score += inciMatches * 12;
    if (inciMatches > 0) evidence.push(`${inciMatches} ingrédients INCI`);
  ...
  calculateDetergentScore(data) {
    ...
    if (productMatches > 0) evidence.push(`${productMatches} produits ménagers détectés`);
    ...
    if (categoryMatches > 0) evidence.push(`${categoryMatches} catégories ménagères`);
    ...
    if (ingredientMatches > 0) evidence.push(`${ingredientMatches} ingrédients détergents`);
