...
${context.available_alternatives ? context.available_alternatives.map(alt => 
  `- ${alt.name}: ${alt.why_better}`
).join('\n') : 'Aucune alternative chargée'}

INSIGHTS SCIENTIFIQUES PERTINENTS:
${context.key_insights ? context.key_insights.map(insight => 
  `- ${insight.title}: ${insight.fact} (Source: ${insight.source})`
...
${context.conversation_history.map(entry => 
