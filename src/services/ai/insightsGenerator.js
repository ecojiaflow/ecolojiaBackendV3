...
class InsightsGenerator {
  ...
  createUltraTransformationInsight(context) {
    ...
    if (ultraTransformationFacts.length === 0) return null;
  ...
  createAdditivesInsight(context) {
    ...
    if (additivesFacts.length === 0 || !context.additives || context.additives.length === 0) return null;

    const detectedAdditive = context.additives[0]; // Premier additif détecté
    let relevantFact = additivesFacts.find(fact => 
      fact.fact.includes(detectedAdditive.code) || fact.explanation.includes(detectedAdditive.code)
    );
  ...
  createGlycemiaInsight(context) {
    ...
    if (glycemiaFacts.length === 0) return null;

    const relevantFact = glycemiaFacts.find(fact => 
      fact.fact.includes('index glycémique') || fact.explanation.includes('glycémique')
    ) || glycemiaFacts[0];
  ...
  createMicrobiomeInsight() {
    ...
    if (microbiomeFacts.length === 0) return null;
  ...
  getPersonalizedInsights(userProfile, productData) {
    ...
    userInterests.forEach(interest => {
      if (triggers[interest]) {
        ...
        if (personalizedInsight) insights.push(personalizedInsight);
      }
    });
  ...
  createPersonalizedInsight(interest, triggerData, productData) {
    ...
    if (specificInsights.length === 0) return null;
  ...
  sortInsightsByRelevance(insights, productData, userProfile) {
    return insights.sort((a, b) => {
      ...
      if (b.personalized && !a.personalized) return 1;
  ...
  categorizeProduct(productData) {
    ...
    if (/yaourt|lait|fromage|kéfir|skyr/.test(name)) return 'dairy';
    if (/jus|boisson|soda|eau/.test(name)) return 'beverages';
    if (/biscuit|chips|barr|snack/.test(name)) return 'snacks';
    if (/céréales|muesli|granola/.test(name)) return 'breakfast';
