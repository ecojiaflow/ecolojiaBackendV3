...
class EcoScoreService {
     ...
     */
    static async calculateDetailedEcoScore(productData) {
        try {
     ...
     */
    static calculateAdvancedConfidence(text, scores) {
        ...
        if (scores.certificationsScore > 0.1)
            confidence += 0.15;
        if (scores.certificationsScore > 0.15)
            confidence += 0.1;
        // Bonus pour origine locale
        if (scores.originScore > 0.05)
            confidence += 0.1;
     ...
     */
    static generateScoreDetails(breakdown) {
        const parts = [];
     ...
     */
    static simulateOCRAnalysis(photos) {
