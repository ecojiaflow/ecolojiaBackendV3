...
exports.DetergentClassifier = void 0;
class DetergentClassifier {
    static async analyzeComposition(composition) {
        const ingredients = composition.toUpperCase().split(/[,;]/).map(s => s.trim());
        const environmentalRisks = [];
        let totalImpactScore = 0;
        let biodegradabilityScore = 100;
        let aquaticToxicityLevel = 'low';
        ...
        const recommendations = this.generateEnvironmentalRecommendations(ecoGrade, environmentalRisks, biodegradabilityScore);
    ...
    static calculateEcoGrade(impactScore, biodegradabilityScore) {
        if (impactScore === 0 && biodegradabilityScore >= 90)
            return 'A';
        if (impactScore <= 25 && biodegradabilityScore >= 70)
            return 'B';
        if (impactScore <= 60 && biodegradabilityScore >= 40)
            return 'C';
        return 'D';
    }
    static evaluatePackaging(ecoGrade) {
    ...
    static generateEnvironmentalRecommendations(grade, risks, biodegradability) {
    ...
    static calculateAnalysisConfidence(ingredientCount) {
    ...
    static async analyzeLaundryDetergent(composition) {
    ...
    static async analyzeDishwashingLiquid(composition) {
        const result = await this.analyzeComposition(composition);
    ...
    static async analyzeAllPurposeCleaner(composition) {
        const result = await this.analyzeComposition(composition);
...
exports.DetergentClassifier = DetergentClassifier;
// Substances problématiques pour l'environnement
DetergentClassifier.HARMFUL_SUBSTANCES = {
    'PHOSPHATES': {
...
DetergentClassifier.ECO_INGREDIENTS = [
