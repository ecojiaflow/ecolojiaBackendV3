...
exports.CosmeticClassifier = void 0;
class CosmeticClassifier {
    ...
    static calculateSafetyGrade(riskScore, concerningCount) {
        if (riskScore === 0)
            return 'A';
        if (riskScore <= 20 && concerningCount <= 2)
            return 'B';
        if (riskScore <= 50 && concerningCount <= 4)
            return 'C';
        return 'D';
    }
    static generateRecommendations(grade, concerning, benefits) {
    ...
    static calculateConfidence(totalIngredients, analyzedIngredients) {
    ...
    static async analyzeSkinCare(ingredients) {
    ...
    static async analyzeHairCare(ingredients) {
        const result = await this.analyzeIngredients(ingredients);
...
exports.CosmeticClassifier = CosmeticClassifier;
// Base de données des ingrédients préoccupants
CosmeticClassifier.CONCERNING_INGREDIENTS = {
    'PARABEN': {
...
CosmeticClassifier.BENEFICIAL_INGREDIENTS = [
