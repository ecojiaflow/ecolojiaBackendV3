...
exports.ultraTransformAnalyzer = exports.UltraTransformAnalyzer = void 0;
// PATH: backend/src/services/ai/ultraTransformAnalyzer.ts
const Logger_1 = require("../../utils/Logger");
const log = new Logger_1.Logger('UltraTransform');
...
class UltraTransformAnalyzer {
    ...
    calcScore(methods, markers) {
        ...
        // Ajustements
        if (methods.some(m => m.cat === 'chem'))
            score += 0.5;
        if (markers.filter(m => m.desc.includes('Additif')).length > 5)
            score += 0.5;
        if (methods.some(m => m.impact === 'E'))
            score = Math.max(score, 4.5);
        return Math.min(10, Math.round(score));
    }
    calcImpact(methods, score) {
...
exports.ultraTransformAnalyzer = new UltraTransformAnalyzer();
