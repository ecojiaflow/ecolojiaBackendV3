...
class DetergentsAnalyzer {
    ...
    identifyProductType(productData) {
        const text = JSON.stringify(productData).toLowerCase();
        if (text.includes('lessive') || text.includes('laundry'))
            return 'laundry_detergent';
        if (text.includes('vaisselle') || text.includes('dish'))
            return 'dishwashing_liquid';
        if (text.includes('sol') || text.includes('floor'))
            return 'floor_cleaner';
