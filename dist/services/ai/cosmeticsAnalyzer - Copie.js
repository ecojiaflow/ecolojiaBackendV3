...
exports.cosmeticsAnalyzer = exports.CosmeticsAnalyzer = void 0;
// PATH: backend/src/services/ai/cosmeticsAnalyzer.ts
const Logger_1 = require("../../utils/Logger");
const log = new Logger_1.Logger('CosmeticsAnalyzer');
...
class CosmeticsAnalyzer {
    async analyzeINCI(ingredients) {
        ...
        const safe = norm.filter(i => !eds.includes(i) && !algs.includes(i) && !probs.some(p => p.name === i));
    ...
    detectNatural(ings) {
        return ings.filter(i => NAT.some(n => i.includes(n)) ||
            /extract|fruit|leaf|root|seed|flower|oil|bio|natural/i.test(i)).length;
    }
    calcHazard(eds, algs, probs) {
        let score = 0;
...
exports.CosmeticsAnalyzer = CosmeticsAnalyzer;
exports.cosmeticsAnalyzer = new CosmeticsAnalyzer();
