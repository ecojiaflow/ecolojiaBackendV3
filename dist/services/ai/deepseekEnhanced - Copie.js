...
exports.DeepSeekEnhanced = void 0;
/**
...
class DeepSeekEnhanced {
    constructor() {
        this.maxTokens = 1000;
        this.temperature = 0.3; // Plus factuel que créatif
     ...
     */
    async generateConversationalResponse(input) {
        if (!this.apiKey) {
     ...
     */
    buildEnhancementSystemPrompt(input) {
...
MISSION: Fournir analyse approfondie basée exclusivement sur science validée.`;
     ...
     */
    buildConversationalMessages(input) {
        const messages = [];
...
- Allergies: ${input.userProfile.allergies.join(', ')}`;
     ...
     */
    generateSuggestedQuestions(input) {
        const questions = [];
    ...
    getFallbackConversationalResponse(input) {
        return {
    ...
    generateDefaultAnalysisQuery(input) {
...
Focus sur données factuelles avec sources récentes.`;
    ...
    containsRiskKeywords(text) {
        ...
        return riskKeywords.some(keyword => text.toLowerCase().includes(keyword.toLowerCase()));
    }
    containsAlternativeKeywords(text) {
        const altKeywords = [
            'alternative', 'remplacer', 'substitut', 'plutôt', 'préférer',
            'naturel', 'bio', 'maison', 'traditioanel', 'artisanal'
        ];
        return altKeywords.some(keyword => text.toLowerCase().includes(keyword.toLowerCase()));
    }
    containsScientificKeywords(text) {
        const sciKeywords = [
            'étude', 'recherche', 'selon', 'mécanisme', 'analyse',
            'BMJ', 'Nature', 'Lancet', 'ANSES', 'EFSA', 'INSERM'
        ];
        return sciKeywords.some(keyword => text.toLowerCase().includes(keyword.toLowerCase()));
    }
    /**
...
exports.DeepSeekEnhanced = DeepSeekEnhanced;
exports.default = DeepSeekEnhanced;
