...
const AuthService_1 = require("../auth/services/AuthService"); // ✅ Import corrigé
...
router.post('/register', registerValidation, async (req, res) => {
    try {
        ...
        const result = await AuthService_1.authService.register({ email, password, name }, sessionInfo);
        if (!result.success) {
        ...
        let emailSent = false;
        if (result.user) {
            emailSent = await EmailService_1.emailService.sendVerificationEmail(result.user.id, result.user.email, result.user.name);
        }
...
router.post('/login', loginValidation, async (req, res) => {
    try {
        ...
        const result = await AuthService_1.authService.login({ email, password }, sessionInfo);
        if (!result.success) {
...
router.post('/logout', async (req, res) => {
    try {
        ...
        const result = await AuthService_1.authService.logout(token);
