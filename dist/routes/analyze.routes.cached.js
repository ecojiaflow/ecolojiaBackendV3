 ...
 */
router.post('/auto', cacheAuthMiddleware_1.publicRouteRateLimit, // Protection DDoS
async (req, res) => {
    try {
        ...
        const cachedResult = await CacheService_1.cacheService.getAnalysis(productData, 'auto');
        if (cachedResult) {
        ...
        switch (detectedType) {
            ...
            case 'cosmetic':
                ...
                break;
            case 'detergent':
                analysisResult = await detergentScorer.analyzeDetergent(productData.ingredients || productData.composition, productData.product_name || '', []);
                break;
            default:
        ...
        await CacheService_1.cacheService.cacheAnalysis(productData, 'auto', response, CACHE_TTL.auto);
 ...
 */
router.post('/food', cacheAuthMiddleware_1.cacheAuthMiddleware, // Auth avec cache Redis
(0, cacheAuthMiddleware_1.checkQuotaMiddleware)('analysis'), // Vérification quota 20/mois
async (req, res) => {
    try {
        const authReq = req;
        const startTime = Date.now();
        ...
        // 🚀 VÉRIFIER LE CACHE
        const cachedResult = await CacheService_1.cacheService.getAnalysis(productData, 'food');
        if (cachedResult) {
            const cacheTime = Date.now() - startTime;
        ...
        // 💾 METTRE EN CACHE
        await CacheService_1.cacheService.cacheAnalysis(productData, 'food', response, CACHE_TTL.food);
 ...
 */
router.post('/cosmetic', cacheAuthMiddleware_1.cacheAuthMiddleware, (0, cacheAuthMiddleware_1.checkQuotaMiddleware)('analysis'), async (req, res) => {
    try {
        const authReq = req;
        const startTime = Date.now();
        ...
        // 🚀 VÉRIFIER LE CACHE
        const cachedResult = await CacheService_1.cacheService.getAnalysis(productData, 'cosmetic');
        if (cachedResult) {
            const cacheTime = Date.now() - startTime;
        ...
        // 💾 METTRE EN CACHE
        await CacheService_1.cacheService.cacheAnalysis(productData, 'cosmetic', response, CACHE_TTL.cosmetic);
 ...
 */
router.post('/detergent', cacheAuthMiddleware_1.cacheAuthMiddleware, (0, cacheAuthMiddleware_1.checkQuotaMiddleware)('analysis'), async (req, res) => {
    try {
        const authReq = req;
        const startTime = Date.now();
        ...
        // 🚀 VÉRIFIER LE CACHE
        const cachedResult = await CacheService_1.cacheService.getAnalysis(productData, 'detergent');
        if (cachedResult) {
            const cacheTime = Date.now() - startTime;
        ...
        // Analyse complète
        const analysisResult = await detergentScorer.analyzeDetergent(productData.ingredients, productData.name || '', productData.certifications);
        if (analysisResult.confidence < 0.4) {
            return res.status(422).json({
                success: false,
                error: 'Analyse non fiable'
        ...
        // 💾 METTRE EN CACHE
        await CacheService_1.cacheService.cacheAnalysis(productData, 'detergent', response, CACHE_TTL.detergent);
 ...
 */
router.post('/ultra-transform', cacheAuthMiddleware_1.publicRouteRateLimit, async (req, res) => {
    try {
        const startTime = Date.now();
        ...
        const cachedResult = await CacheService_1.cacheService.getAnalysis(productData, 'ultra-transform');
        if (cachedResult) {
            const cacheTime = Date.now() - startTime;
        ...
        await CacheService_1.cacheService.cacheAnalysis(productData, 'ultra-transform', response, CACHE_TTL.ultraTransform);
 ...
 */
router.get('/stats', cacheAuthMiddleware_1.cacheAuthMiddleware, async (req, res) => {
    try {
        const authReq = req;
 ...
 */
router.post('/cache/clear', cacheAuthMiddleware_1.cacheAuthMiddleware, async (req, res) => {
    try {
        const authReq = req;
        ...
        const deleted = await CacheService_1.cacheService.invalidatePattern(pattern);
        res.json({
            success: true,
 ...
 */
async function analyzeWithFoodScorer(productData, userProfile = {}) {
 ...
 */
function personalizeAnalysis(analysis, userProfile) {
 ...
 */
function calculateNutritionalImpact(ultraResult) {
 ...
 */
function generateUltraTransformRecommendations(ultraResult) {
 ...
 */
function calculateNaturalityMatrix(ingredients, ultraResult) {
