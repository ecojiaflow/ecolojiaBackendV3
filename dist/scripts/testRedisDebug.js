...
async function debugRedisConnection() {
    ...
    try {
        const redis1 = new ioredis_1.default(process.env.REDIS_URL, {
    ...
    try {
        ...
        const redis2 = new ioredis_1.default(urlWithTls, {
            maxRetriesPerRequest: 1,
            connectTimeout: 5000,
            enableReadyCheck: false,
            tls: {
    ...
    try {
        const urlParts = process.env.REDIS_URL?.match(/redis:\/\/([^:]+):([^@]+)@([^:]+):(\d+)/);
        if (!urlParts)
            throw new Error('URL Redis invalide');
        const redis3 = new ioredis_1.default({
            host: urlParts[3],
